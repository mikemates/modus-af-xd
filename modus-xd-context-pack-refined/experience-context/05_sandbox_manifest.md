...
**Coverage gaps** (routes or states intentionally not built yet):

| Missing route / state | Why missing | Needed before backlog? | Owner |
|---|---|---:|---|
| [Item] | [Reason] | Yes / No | [Owner] |
...
Principles: use representative data (not happy-path-only); include edge cases that affect layout, validation, status, or decisions (empty, error, permission, large-list/stress); mark fake data clearly; no real client-confidential or regulated data unless approved; align object names with `04_ia_object_model.md`. Keep example object JSON with the sandbox code, not here.

| Data set | Purpose | Objects represented | Edge cases represented (empty/error/permission/stress) | Source file |
|---|---|---|---|---|
...
- Experience Sandbox README (setup, code hygiene, how eng/agents use it): `../experience-sandbox/README.md`
- Drift Log: `../experience-sandbox/drift_log.md`
- AI Readiness Gate: `../templates-and-standards/experience_ai_readiness_gate.md`

This manifest is the single source for route/flow/state coverage, mock-data coverage, and reusable-vs-reference mapping. The sandbox folder holds only what ships with the code (README, drift log).
