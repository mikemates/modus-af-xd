...
**Inherits Modus Experience Standards.** Default conventions (prefer accessible library components, custom requires review, tokens over hard-coded values, and the default UI foundation) come from the Modus Experience Standards. Capture only this product's specific components, adaptations, gaps, and constraints here.
