...
Author stories with `experience_story_extension.md` (the template); use this gate to validate their readiness. Both cover the same field set for different purposes — do not maintain a third copy.
...
This gate checks whether a *ticket* carries enough context to build. It uses the same experience quality dimensions defined in `experience_quality_review.md` — that table is the shared source; the review file applies the dimensions to sandboxes and built software, this gate applies them to backlog items. Keep the three in sync.
...
Standards live in `../experience-context/09_accessibility_usability.md`; verify the story states what applies to it:

- [ ] Keyboard, focus, and screen-reader expectations are defined where relevant.
...
Event naming and signal guidance live in `../experience-context/11_experience_measurement.md`; verify the story applies them:

- [ ] Story includes a measurement hypothesis if behavior change is expected.
- [ ] Required event(s) are named with clear triggers and properties.
